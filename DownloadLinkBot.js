...
bot.onText(/\/start/, (msg) => {
    ...
    bot.sendMessage(chatId, 'Hi! Please send me a link.');
});
...
async function generateDirectDownloadLink(url) {
    ...
    // Modify this logic according to the structure of the page
    $('a').each((index, element) => {
        const href = $(element).attr('href');
        if (href) {
            directDownloadLink = href;
            return false; // Exit the loop once the first link is found
        }
    });
